...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === 'GET' && req.url === '/dogs') {
      // Your code here 
      const htmlPage = fs.readFileSync("./views/dogs.html", 'utf-8'); // how do you know to change this?
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        const resBody = htmlPage.replace(/#{name}/g, dog.name).replace(/#{age}/g, dog.age) //g matches everything within the page, not just the 1st instance;
        res.statusCode = 200;
        res.setHeader('Content-Type', 'text/html');
        res.write(resBody);
        return res.end();
